'use strict';
/*
 * Simple app implementation to test the current version of passport-aloha against
 */
...
//
//   https://localhost:3000/auth/aloha/callback
// 
// then, you should have a HTTP GET endpoint like:
//
//   app.get('/auth/aloha/callback, callback))
//
var CF_CALLBACK_URL = 'http://localhost:3000/auth/aloha/callback';
...
var sfStrategy = new AlohaStrategy({
  ...
  process.nextTick(function() {
    ...
    // To keep the example simple, the user's salesforce profile is returned to
    // represent the logged-in user.  In a typical application, you would want
    // to associate the salesforce account with a user record in your database,
...
//   redirect the user back to this application at /auth/aloha/callback
app.get('/auth/aloha', passport.authenticate('aloha'), function(req, res) {
  // The request will be redirected to Force.com for authentication, so this
  // function will not be called.
});

// GET /auth/aloha/callback
//   PS: This MUST match what you gave as 'callback_url' earlier
//   Use passport.authenticate() as route middleware to authenticate the
//   request.  If authentication fails, the user will be redirected back to the
//   login page.  Otherwise, the primary route function function will be called,
//   which, in this example, will redirect the user to the home page.
app.get('/auth/aloha/callback', passport.authenticate('aloha', {
